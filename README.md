...
| 3D 桌宠 | SceneKit 桌宠“川仔”持续漂浮、转身并响应鼠标；点击查看对话记忆 |
| 实时语音 | 按住语音键对话，实时显示用户转录与 AI 回复字幕，并使用 Vivi 音色流式播放 |
...
设置页支持蓝牙自动连接、重新扫描、忘记设备，以及显示或隐藏桌宠。旋钮选择项目，Enter、
Action 或旋钮按下执行；所有设置通过 `NSUserDefaults` 持久化。
